...
namespace UnityProjectArchitect.Services.AI
{
    public class ClaudeAPIClient
    {
        ...
        {
            _keyManager = APIKeyManager.Instance;
        ...
        private async Task<ClaudeAPIResponse> SendRequestWithRetry(ClaudeAPIRequest request)
        {
            ...
            for (int attempt = 0; attempt <= maxRetries; attempt++)
            {
                try
                {
                    ...
                    {
                        Debug.LogWarning($"Non-retryable error: {response.error.message}");
                        return response;
                    }

                    if (attempt == maxRetries)
                    {
                        Debug.LogError($"Max retries ({maxRetries}) exceeded");
                        return response;
                    }

                    Debug.LogWarning($"Request failed (attempt {attempt + 1}), retrying: {response.error?.message}");
                }
                catch (Exception ex)
                {
                    if (attempt == maxRetries)
                    ...
                    Debug.LogWarning($"Request attempt {attempt + 1} failed with exception: {ex.Message}");
                }
            }
        ...
        private async Task<ClaudeAPIResponse> SendSingleRequest(ClaudeAPIRequest request)
        {
            ...
            using (UnityWebRequest webRequest = new UnityWebRequest(_configuration.baseUrl, "POST"))
            {
                ...
                webRequest.timeout = _configuration.timeoutSeconds;
                ...
                {
                    OnProgress?.Invoke(0.3f + (operation.progress * 0.6f));
                    await Task.Delay(50);
                }
                ...
                {
                    Debug.Log($"Claude API request completed in {requestTime.TotalSeconds:F2}s");
                }

                UpdateRateLimitInfo(webRequest.GetResponseHeaders());
                ...
                if (webRequest.result == UnityWebRequest.Result.Success)
                {
                    ...
                    try
                    {
                        ClaudeAPIResponse response = JsonUtility.FromJson<ClaudeAPIResponse>(responseJson);
                        if (response == null)
                        {
                            return new ClaudeAPIResponse
                            {
                        ...
                        {
                            Debug.Log($"Claude API success: {response.usage?.TotalTokens ?? 0} tokens used");
                        }
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(webRequest.downloadHandler.text))
                    {
                        try
                        {
                            ClaudeAPIResponse errorResponse = JsonUtility.FromJson<ClaudeAPIResponse>(webRequest.downloadHandler.text);
                        ...
                        {
                            errorMessage += $" - Response: {webRequest.downloadHandler.text}";
                        }
                    }
        ...
        public void UpdateConfiguration(ClaudeConfiguration newConfiguration)
        {
            ...
            {
                Debug.LogError("Cannot update with invalid configuration");
                return;
            }
            ...
            Debug.Log("Claude API configuration updated successfully");
        }
        ...
        public async Task<string> GetQuickResponseAsync(string userMessage, string systemPrompt = null)
        {
            try
            {
                ...
                else
                {
                    ...
                    Debug.LogError($"Quick response failed: {errorMessage}");
                    return null;
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"Quick response exception: {ex.Message}");
                return null;
            }
        }
        ...
        public void ClearCache()
        {
            lock (_requestLock)
            {
                ...
                Debug.Log("Claude API client cache cleared");
            }
        }
