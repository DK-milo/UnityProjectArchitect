...
namespace UnityProjectArchitect.Core.AI
{
    ...
    public class ClaudeRateLimitInfo
    {
        ...
        public void UpdateFromHeaders(Dictionary<string, string> headers)
        {
            ...
            {
                if (int.TryParse(headers["x-ratelimit-requests-remaining"], out int tempRequests))
                {
                    remainingRequests = tempRequests;
                }
            }

            if (headers.ContainsKey("x-ratelimit-tokens-remaining"))
            {
                if (int.TryParse(headers["x-ratelimit-tokens-remaining"], out int tempTokens))
                {
                    remainingTokens = tempTokens;
                }
            }
